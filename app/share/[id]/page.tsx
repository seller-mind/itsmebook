    ...
    <div className="min-h-screen bg-gradient-to-b from-orange-50 via-amber-50 to-purple-50">
      ...
      <div className="bg-white/80 backdrop-blur-sm shadow-sm sticky top-0 z-50">
        <div className="max-w-lg mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <a href="/" className="px-3 py-1.5 bg-orange-100 text-orange-700 rounded-full text-sm font-medium hover:bg-orange-200 transition-colors">
            📖 创作我的
          </a>
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto px-4 py-6">
        ...
        <div className="bg-white rounded-3xl shadow-xl overflow-hidden">
          ...
          {/* 音频进度条（仅有声版） */}
          {hasAudio && currentPageData.audio_url && isPlaying && (
        ...
        <div className="mt-6 flex items-center justify-center gap-4">
          <button onClick={prevPage} disabled={currentPage === 0}
            className={`w-12 h-12 rounded-full flex items-center justify-center text-xl transition-all ${currentPage === 0 ? "bg-gray-200 text-gray-400 cursor-not-allowed" : "bg-white shadow-md text-gray-700 hover:bg-orange-50"}`}>
            ‹
          </button>
          ...
          {hasAudio ? (
            ...
            /* 无声版：只显示翻页，不显示播放按钮 */
            <div className="w-16 h-16 rounded-full flex items-center justify-center text-lg font-medium text-gray-400 bg-gray-100">
              {currentPage + 1}
            </div>
          )}

          <button onClick={nextPage} disabled={isEnd}
            className={`w-12 h-12 rounded-full flex items-center justify-center text-xl transition-all ${isEnd ? "bg-gray-200 text-gray-400 cursor-not-allowed" : "bg-white shadow-md text-gray-700 hover:bg-orange-50"}`}>
            ›
          </button>
        </div>
